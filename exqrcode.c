...
enum imageType {
	PNG_TYPE, EPS_TYPE, ANSI_TYPE, ANSI256_TYPE, ASCII_TYPE, ASCIIi_TYPE
};
...
static QRcode *encode(const unsigned char *intext, int length) {
	...
	if (micro) {
		...
		if (eightbit) {
			...
			code = QRcode_encodeString((char *) intext, version, level, hint,
					casesensitive);
		}
	}
...
PHP_METHOD( ExQrcode, encodeMECard) {
	...
	for (zend_hash_internal_pointer_reset_ex(arr_hash, &pointer); zend_hash_get_current_data_ex(
			arr_hash, (void**) &data, &pointer) == SUCCESS; zend_hash_move_forward_ex(
			arr_hash, &pointer)) {
		...
		if (zend_hash_get_current_key_ex(arr_hash, &key, &key_len, &index, 0,
			...
			if (strcmp("N", key) == 0) {
				tm_text_len += sprintf(tm_text + tm_text_len, "N:%s;",
						Z_STRVAL_PP(data));
				//php_printf("lengthof name %d %d\n",strlen(Z_STRVAL_PP(data)),Z_STRLEN_PP(data));
				//break;
			} else if (strcmp("TEL", key) == 0) {
				tm_text_len += sprintf(tm_text + tm_text_len, "TEL:%s;",
						Z_STRVAL_PP(data));

			} else if (strcmp("EMAIL", key) == 0) {
				tm_text_len += sprintf(tm_text + tm_text_len, "EMAIL:%s;",
						Z_STRVAL_PP(data));

			} else if (strcmp("ADDR", key) == 0) {
				tm_text_len += sprintf(tm_text + tm_text_len, "ADDR:%s;",
						Z_STRVAL_PP(data));

			} else if (strcmp("URL", key) == 0) {
				tm_text_len += sprintf(tm_text + tm_text_len, "URL:%s;",
						Z_STRVAL_PP(data));
			} else {
				;
			}
...
#endif
		"exqrcode", exqrcode_functions, PHP_MINIT(exqrcode), PHP_MSHUTDOWN(
				exqrcode), PHP_RINIT(exqrcode), /* Replace with NULL if there's nothing to do at request start */
